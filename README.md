...
This project uses Playwright and Node.js to crawl the Quip browse UI, walk through accessible folders, gather document links from each folder, and export each document to HTML while preserving the folder hierarchy locally.
...
4. Opens one folder page at a time and records the current folder path.
5. Collects subfolder links from the current folder page and queues them for traversal.
6. Collects document links from the current folder page and associates them with that folder path.
7. Deduplicates documents by normalized URL.
8. Opens each document once.
9. Opens the document menu and triggers `Export -> HTML`.
10. Saves the Playwright download into `exports/<folder path>/` using a sanitized filename.
11. Records successful exports in `exports/.quip-export-state.json`.
...
- The script preserves folder structure based on the browse folder page where each document link was discovered.
- The script only exports documents it can discover from folder pages rendered by Quip. If some folders or lists are lazy-loaded differently, you may need to adjust the browse selectors or start from a different Quip page.
